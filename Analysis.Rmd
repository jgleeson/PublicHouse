---
title: "Analysing the PublicHouse dataset, v3"
...
```{r dwellings and population in UK and France}
d %>%
...
```{r aggregate dwellings per person trend by country grouping, fig.height = 5, fig.width= 7}
d_interpolated %>%
  pivot_wider(id_cols = c(area_name, year, area_level, grouping),
              names_from = variable,
              values_from = value) %>%
...
```{r dwellings per person animation 1, fig.height = 7, fig.width=9, eval = F, include=F}
p <- d_interpolated %>%
  pivot_wider(id_cols = c(area_name, year, area_level, grouping),
              names_from = variable,
              values_from = value) %>%
...
#  arrange(area_name, year) %>%
  ...
  group_by(year) %>%
#  mutate(rank = min_rank(-dpp) * 1) %>%
  mutate(rank = rank(-dpp)) %>% 
#  group_by(area_name) %>% 
#  arrange(year, dpp) %>%
#  mutate(rank = 1:n()) %>%
  ggplot(aes(y = dpp, x = as.numeric(as.factor(rank)), fill = grouping)) +
  geom_col() +
  scale_fill_okabeito() +
  scale_x_continuous() + 
  labs(x = NULL, y = NULL, title = "Dwellings per 1,000 people by country") +
...
#animate(p + transition_time(year), nframes = 100, type = "cairo")
animate(p + transition_states(year, transition_length = 4, state_length = 1) +
          view_follow(fixed_y = T) + ease_aes('cubic-in-out'), nframes = 100, type = "cairo") 
# 

# https://evamaerey.github.io/little_flipbooks_library/racing_bars/racing_barcharts.html#35
